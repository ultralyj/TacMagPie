...
def create_model_xml(grid=config.GRID_SIZE, indenter=config.INDENTER_RADIUS):
    ...
    # log_info(f"柔性体质量: {mass:.6f} kg")

    xml_content = f'''
    <mujoco model="Press">
        <include file="scene.xml" />
        <compiler autolimits="true" />
        <option solver="Newton" tolerance="1e-5" timestep="{config.TIMESTEP}" 
        ...
        <worldbody>
            ...
            <body name="indenter">
                ...
                <site name="force_sensor_site" type="sphere" size="0.005" pos="{config.INDENTER_X} {config.INDENTER_Y} 0.005" rgba="1 0 0 1"/>
            </body>
